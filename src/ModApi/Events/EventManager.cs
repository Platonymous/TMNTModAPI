...
namespace ModLoader.Events
{
    internal class EventManager : IEventHelper
    {
        ...
        internal void Init()
        {
            ...
            {
                TMNTModApi.Singleton.modHelper.Console.Error(e.Message);
        ...
        {
            if (__result == __instance.GlobalContentManager)
            {
                ...
                __result = __instance.GlobalContentManager;
            }
            else if (!(__result is ApiContentManager))
                __result = new ParisContentManager(__result.ServiceProvider, __result.RootDirectory);
        }
